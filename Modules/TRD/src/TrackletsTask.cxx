...
void TrackletsTask::drawHashOnLayers(int layer, int hcid, int rowstart, int rowend)
{
  ...
  std::cout << "Tracklet Box for hcid : " << hcid << " sec: " << sec << " side: " << side << " layer: " << layer << " = " << bottomleft.first << ":" << bottomleft.second << " -- " << topright.first << ":" << topright.second << std::endl;
    ...
    for (int iSec = 0; iSec < 18; ++iSec) {
      for (int iStack = 0; iStack < 5; ++iStack) {
        ...
        for (int side = 0; side < 8; ++side) {
          ...
          if (mChamberStatus->isMasked(hcid) && (!hcidone.test(hcid)) {
